...
// Extracted and memoized so quantity +/- in Cart does not re-render this section
const RecommendedProducts = React.memo(({ recommendations, recsLoading, onViewProduct }) => {
    if (!recsLoading && recommendations.length === 0) return null;
...
const Cart = () => {
    ...
    // Derive firstProductId as a primitive — this does NOT change when quantity updates,
    // only when the first cart item itself changes. This prevents re-fetching recommendations
    // on every + / - click.
    ...
    }, [firstProductId]); // only re-fetches when first product changes, not on quantity update

    const handleViewProduct = useCallback((rec) => {
        setSelectedProduct({
            id: rec.productId,
